...
def main(model_id, sampler_name, k, with_class, device, alias):
    ...
    if with_class:
        results.to_csv(f"data/results/results_{alias}_{sampler_name}_{k}_in_class.csv", index=False)
    else:
        results.to_csv(f"data/results/resultus_{alias}_{sampler_name}_{k}_general.csv", index=False)
        print("Saved")
    print("Done!")
    return
