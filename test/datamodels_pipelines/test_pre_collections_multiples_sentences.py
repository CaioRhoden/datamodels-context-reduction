...
class TestPipelinePreCollectionMultipleSentencesSingleInference:
    ...
    def setup_class(cls):
        ...
        single_inference_pipeline = BaseLLMPreCollectionsPipeline(
            ...
            log = False,
            log_config = None,
            model_configs=single_model_configs,
        )
        pipe.create_pre_collection(single_inference_pipeline)
    ...
    def test_predicted_output_length(self):
        df = pl.read_ipc(f"{self.datamodels_path}/pre_collections/train/pre_collection_1.feather")
        assert (df["predicted_output"].list.len() == 3).all()
...
class TestPipelinePreCollectionMultipleSentencesGenericInstructBatchHF:
    ...
    def setup_class(cls): 
        ...
        pre_collection_pipeline = BatchLLMPreCollectionsPipeline(
            ...
            mode="train",
            instruction="Answer",
            model=GenericInstructBatchHF(f"{PATH}/{os.environ['DATAMODELS_TEST_MODEL']}", quantization="fp16"),
            context_strategy=fill_prompt_template,
            rag_indexes_path=f"{tmp_path}/indexes.json",
            output_column="answer",
            batch_size=3,
...
class TestPipelinePreCollectionMultipleSentencesBatchInferenceVLLM:
