...
class StratifiedSetter(BaseSetter):
    ...
    def set(self) -> None:
        ...
        ## Get random samples
        random_indices_random = [np.random.choice(random_set.select(self.config.index_col).to_numpy().squeeze(), self.config.k, replace=False) for _ in range(self.config.n_samples_random)]
        ...
        for _ in range(self.config.n_samples_mix):
            ...
            random_indices = np.random.choice(random_set.select(self.config.index_col).to_numpy().squeeze(), n_random, replace=False)
